...
function Toast({ msg, onDone }) {
  ...
  return (
    <div style={{
      ...
      background: "#0f1923", color: "#4ade80", padding: "10px 20px", borderRadius: 24,
      fontSize: 13, fontWeight: 700, border: "1px solid #4ade8040", zIndex: 9999,
      display: "flex", alignItems: "center", gap: 8, whiteSpace: "nowrap",
      boxShadow: "0 8px 32px #0008", animation: "slideUp .25s ease"
    }}>✓ {msg}</div>
  );
}
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#000b", zIndex: 500, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{
        background: "#0b1622", width: "100%", maxWidth: 440,
        borderRadius: "22px 22px 0 0", padding: "24px 20px 36px",
        border: "1px solid #1e2d3d", borderBottom: "none",
        animation: "slideUp .3s ease", maxHeight: "85vh", overflowY: "auto"
      }}>
  ...
  return (
    <Modal title={`${stock.logo} ${stock.id} — ${stock.name}`} onClose={onClose}>
      <div style={{ display: "flex", gap: 8, marginBottom: 18 }}>
        {["buy", "sell"].map(m => (
          <button key={m} onClick={() => setMode(m)} style={{
            ...
            background: mode === m ? (m === "buy" ? "#4ade80" : "#ef4444") : "#1e2d3d",
            color: mode === m ? "#000" : "#64748b", fontWeight: 700, fontSize: 14,
            fontFamily: "'Space Mono',monospace", transition: "all .2s"
          }}>{m === "buy" ? "Купить" : "Продать"}</button>
        ))}
      </div>
      ...
      <button onClick={handle} style={{
        ...
        background: mode === "buy" ? "linear-gradient(135deg,#4ade80,#16a34a)" : "linear-gradient(135deg,#ef4444,#b91c1c)",
        border: "none", borderRadius: 12, padding: "14px", color: "#fff",
        fontWeight: 700, fontSize: 15, cursor: "pointer", fontFamily: "'Space Mono',monospace"
      }}>{mode === "buy" ? "🛒 Купить" : "💸 Продать"}</button>
    </Modal>
  );
}
  ...
  return (
    <Modal title={`Получить ${label}`} onClose={onClose}>
      <div style={{ textAlign: "center" }}>
        ...
        <button onClick={() => { navigator.clipboard?.writeText(address).catch(() => {}); setCopied(true); setTimeout(() => setCopied(false), 2000); }}
          style={{ marginTop: 14, width: "100%", background: copied ? "linear-gradient(135deg,#4ade80,#16a34a)" : "linear-gradient(135deg,#3b82f6,#2563eb)", border: "none", borderRadius: 12, padding: "13px", color: "#fff", fontWeight: 700, cursor: "pointer" }}>
...
function SendModal({ label, symbol, balance, onClose, onSend }) {
  ...
  return (
    <Modal title={`Отправить ${label}`} onClose={onClose}>
      ...
      <button onClick={handle} style={{ marginTop: 16, width: "100%", background: "linear-gradient(135deg,#f7931a,#ea580c)", border: "none", borderRadius: 12, padding: "14px", color: "#fff", fontWeight: 700, fontSize: 15, cursor: "pointer" }}>
        ➤ Отправить
      </button>
    </Modal>
  );
...
function StockRow({ stock, sparkline, portfolio, onTrade, compact }) {
  ...
  return (
    <div onClick={onTrade} style={{
      background: "#0a1520", border: "1px solid #1e2d3d", borderRadius: 14,
      padding: compact ? "10px 12px" : "12px 14px", marginBottom: 8,
...
export default function App() {
  ...
  function handleTrade(mode, stock, shares) {
    ...
    setState(s => {
      ...
      return {
        ...s, usdBalance: newUsd, portfolio: newPortfolio,
        transactions: [{ id: genTx(), type: mode, asset: stock.id, amount: shares, price: stock.price, total: cost, date: now(), status: "confirmed", note: `${mode === "buy" ? "Покупка" : "Продажа"} ${shares} акц. ${stock.id}` }, ...s.transactions]
      };
    });
  ...
  function handleSendCrypto(asset, toAddr, amount) {
    ...
    setState(s => ({
      ...s,
      [key]: +(s[key] - amount).toFixed(asset === "BTC" ? 8 : 2),
      transactions: [{ id: genTx(), type: "send", asset, amount, date: now(), status: "pending", note: `→ ${toAddr.slice(0, 16)}...` }, ...s.transactions]
    }));
